...
for (const name of ['index.html', 'style.css', 'script-proxy.js']) {
  fs.copyFileSync(path.join(projectDir, name), path.join(outputDir, name));
}
...
fs.writeFileSync(path.join(outputDir, '_routes.json'), JSON.stringify({
  version: 1,
  include: ['/*'],
  exclude: ['/style.css', '/script-proxy.js', '/demo/demo.css', '/demo/demo.js']
}, null, 2), 'utf8');

console.log('Built online site with main app and portfolio case page.');
